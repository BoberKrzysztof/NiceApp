...
app_info = {
    'db_file' : '/Users/krzysztofbober/Documents/NicerApp/data/cantor.db'
}
...
class UserPass:
    ...
    def login_user(self):
        db = get_db()
        sql_statement = 'select id, name, email, passowrd, is_active, is_admin from users where name=?;'
        cur = db.execute(sql_statement, [self.user])
        user_record = cur.fetchone()

        if user_record!=None and self.verify_password(user_record['passowrd'], self.password):
            return user_record
        else:
            self.user = None
            self.password = None
...
def init_app():
    ...
    db.execute('insert into users(name, email, passowrd, is_active, is_admin) values(?,?,?,True,True);', [user_pass.user, 'noone@nowhere.no', user_pass.hash_password()])
    db.commit()
...
def exchange():
    ...
    if request.method == 'GET':
        ...
        return render_template('exchange_results.html', active_menu='exchange',currency=currency, amount=amount, currency_info=offer.get_by_code(currency), login=login)
...
def delete_transaction(transaction_id):
    ...
    db.execute(sql_command, [transaction_id])
    db.commit()
...
def edit_user(user_name):
    ...
    if request.method == 'GET':
        ...
        if new_password != '':
            ...
            sql_statement = "update users set passowrd = ? where name = ?;"
...
def new_user():
    ...
    if request.method == 'GET':
        ...
        if not message:
            ...
            sql_statement = 'insert into users(name, email, passowrd, is_active, is_admin) values(?,?,?,True,False);'
